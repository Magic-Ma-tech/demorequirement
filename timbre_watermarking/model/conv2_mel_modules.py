...
from timbre_watermarking.distortions.frequency import TacotronSTFT, fixed_STFT, tacotron_mel
from timbre_watermarking.distortions.dl import distortion
import pdb
import timbre_watermarking.hifigan as hifigan
import json
import torchaudio
...
def get_vocoder(device):
    ...
    with open(HIFIGAN_ROOT / "config.json", "r") as f:
    # with open("/home/binhaoma/local/generation_model_Bench/timbre_watermarking/hifigan/config.json", "r") as f:
        config = json.load(f)
    ...
    ckpt = torch.load(
        f"{HIFIGAN_ROOT}/model/VCTK_V1/generator_v1",
        map_location=device,
    )
