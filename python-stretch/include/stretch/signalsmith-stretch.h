...
struct SignalsmithStretch {
	...
	void setTransposeSemitones(Sample semitones,
		...
		if (flatBandScales.size() % 3 != 0) {
			throw std::runtime_error("bandScales must contain triples of (start, end, scale)");
		}

		customBandScales = std::move(flatBandScales);  // ✅ 直接存扁平列表
	}
...
private:
	...
	void clearCustomBandPitchShifts() {
    customBandScales.clear();
	}
	...
	void updateOutputMap() {
		...
		// for (int b = 0; b < bands; ++b) {
		// 	Sample freq = bandToFreq(b);

		// 	for (int i = 0; i + 2 < static_cast<int>(customBandScales.size()); i += 3) {
		// 		Sample fStart = customBandScales[i];
		// 		Sample fEnd   = customBandScales[i + 1];
		// 		Sample ratio  = customBandScales[i + 2];

		// 		if (freq >= fStart && freq < fEnd) {
		// 			auto &map = outputMap[b];
		// 			Sample originalFreq = freq;
		// 			Sample mappedFreq = originalFreq + (bandToFreq(map.inputBin) - originalFreq) * ratio;
		// 			map.inputBin = freqToBand(mappedFreq);
		// 		}
		// 	}
		// }
		// if (!customBandScales.empty()) {
		// 	std::cout << "Custom pitch shaping active.\n";
		// }
		// if (customBandScales.empty()) {
		// 	std::cout << "Custom pitch is empty.\n";
		// }
